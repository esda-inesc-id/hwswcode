...
Q & AP\_TRN, AP\_RND & What to do when dropping LSBs\\
...
float compute_float(float x[], int n); // float reference
fx_t  compute_fixed(fx_t  x[], int n);// ap_fixed DUT
int main() {
...
\vspace{0.04cm}
\begin{tikzpicture}[
  kbox/.style={rounded corners=4pt, minimum width=6.5cm, minimum height=1.1cm,
               align=left, font=\small, text=white, inner sep=8pt},
  node distance=0.3cm
]
  \node[kbox, fill=Navy] (k1)
    {\textbf{1. Methodology:}
     range $\to$ target $\to$ sweep $W$ $\to$ check II \& area.
     Follow all 4 steps every time.};
  \node[kbox, fill=Orange, below=of k1] (k2)
    {\textbf{2. Accumulator rule:}
     $W_{\text{acc}} = W_{\text{data}} + \lceil\log_2 N\rceil$;\;
     $I_{\text{acc}} = I_{\text{data}} + \lceil\log_2 N\rceil$.
     Never guess --- derive it.};
  \node[kbox, fill=Teal, below=of k2] (k3)
    {\textbf{3. Measure SNR:}
     run a C testbench with real inputs.
     Theory gives you a starting point; simulation gives you the answer.};
  \node[kbox, fill=GreenHi, below=of k3] (k4)
    {\textbf{4. \texttt{ap\_fixed} vs.\ float:}
     fixed-point gives II\,=\,1 and far lower area.
     Choose the smallest $W$ that meets your SNR target.};
\end{tikzpicture}
\end{frame}
